...
def add_subtype():
    ...
    if not all([exercise_id, subtype_name, subtype_id]):
        return jsonify({"success": False, "message": "exercise_id, subtype_name, and subtype_id are required"}), 400

    if exercise_model.add_subtype(exercise_id, subtype_name, subtype_id):
        return jsonify({"success": True, "message": "Subtype added"}), 200
    else:
        return jsonify({"success": False, "message": "Failed to add subtype"}), 500


@exercise_bp.route("/remove_subtype", methods=["POST"])
def remove_subtype():
    data = request.json
    exercise_id = data.get("exercise_id")
    subtype_name = data.get("subtype_name")

    if not all([exercise_id, subtype_name]):
        return jsonify({"success": False, "message": "exercise_id and subtype_name are required"}), 400

    if exercise_model.remove_subtype(exercise_id, subtype_name):
        return jsonify({"success": True, "message": "Subtype removed"}), 200
    else:
        return jsonify({"success": False, "message": "Failed to remove subtype"}), 500


@exercise_bp.route("/update_main_name", methods=["POST"])
